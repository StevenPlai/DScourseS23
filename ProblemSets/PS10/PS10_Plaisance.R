...
# set up the tuning and cross-validation
tree_tune <- tune_grid(
  tree_wf,
  resamples = tree_folds, # number of folds
  grid = tree_parm_df, # hyperparameter grid to search over
  control = control_grid(verbose = TRUE) # print out progress
)

# what is the best value of lambda?
top_acc  <- show_best(tree_tune, metric = "accuracy")
best_acc <- select_best(tree_tune, metric = "accuracy")
...
nnet_tune <- tune_grid(
  nnet_wf,
  resamples = nnet_folds, # number of folds
  grid = nnet_parm_df, # hyperparameter grid to search over
  control = control_grid(verbose = TRUE) # print out progress
)

# what is the best value of lambda?
top_acc  <- show_best(nnet_tune, metric = "accuracy")
best_acc <- select_best(nnet_tune, metric = "accuracy")
...
knn_tune <- tune_grid(
  knn_wf,
  resamples = knn_folds, # number of folds
  grid = knn_parm_df, # hyperparameter grid to search over
  control = control_grid(verbose = TRUE) # print out progress
)

# what is the best value of lambda?
top_acc  <- show_best(knn_tune, metric = "accuracy")
best_acc <- select_best(knn_tune, metric = "accuracy")
...
svm_tune <- tune_grid(
  svm_wf,
  resamples = svm_folds, # number of folds
  grid = svm_parm_df, # hyperparameter grid to search over
  control = control_grid(verbose = TRUE) # print out progress
)

# what is the best value of lambda?
top_acc  <- show_best(svm_tune, metric = "accuracy")
